...
bool Interaccion::condicionjaque(Pieza& t, Casilla s, int& _turno, int& _o)
{
	switch (t.getTipoPieza())
	{
	case 1:
		if (t.getEstado() == 1 && (s.getF() == t.getfila() || s.getC() == t.getcolumna()) && t.getColor() == _turno && _turno != _o) //Movimiento de las torres blancas y negras
			return true;
		break;

	case 2:
		if (t.getEstado() == 1 && ((abs(s.getC() - t.getcolumna()) == 1 && abs(s.getF() - t.getfila()) == 2) || (abs(s.getC() - t.getcolumna()) == 2 && abs(s.getF() - t.getfila()) == 1)) && t.getColor() == _turno && _turno != _o) //Movimiento de los caballos blancos y negros
			return true;
		break;

	case 3:
		if (t.getEstado() == 1 && (abs(s.getF() - t.getfila()) == abs(s.getC() - t.getcolumna())) && t.getColor() == _turno && _turno != _o) //Movimiento de los alfiles blancos y negros
			return true;
		break;

	case 4:
		if (t.getEstado() == 1 && (s.getF() == t.getfila() || s.getC() == t.getcolumna() || (abs(s.getF() - t.getfila()) == abs(s.getC() - t.getcolumna()))) && t.getColor() == _turno && _turno != _o)	//Movimiento de las damas blancas y negras
			return true;
		break;

	case 5:
		if (t.getEstado() == 1 && (abs(s.getC() - t.getcolumna()) <= 1 && abs(s.getF() - t.getfila()) <= 1) && t.getColor() == _turno && _turno != _o)	//movimiento de los reyes blanco y negro
			return true;
		break;

	case 6:
		if (t.getColor() == 0 && t.getEstado() == 1 && ((s.getC() - t.getcolumna()) == 1) && ((s.getF() - t.getfila()) == 0) && t.getColor() == _turno && _turno != _o) //Movimiento peones blancos
			return true;
		if (t.getColor() == 1 && t.getEstado() == 1 && ((t.getcolumna() - s.getC()) == 1) && ((s.getF() - t.getfila()) == 0) && t.getColor() == _turno && _turno != _o) //Movimiento peones negros
			return true;
		break;
	}
